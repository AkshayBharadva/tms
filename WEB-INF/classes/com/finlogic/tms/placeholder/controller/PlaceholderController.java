...
//            CommonMember.appendLogFile("Download file Successfully ");
    ...
    public ModelAndView uploadExcelFileWithData(HttpServletRequest request , HttpServletResponse response ,@RequestParam("fileupload") CommonsMultipartFile file)
    {
        ModelAndView modelAndView = new ModelAndView("placeholderStatus");
        String fileName = file.getOriginalFilename();
        try {
            
            file.transferTo(new File(CommonUtil.storageFilePath() + fileName));
            modelAndView.addObject("status", "1");
            modelAndView.addObject("Action", "fileUploadMsg");
